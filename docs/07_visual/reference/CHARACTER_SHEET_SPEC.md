...
LoRA 合成前に、全キャラについて `theo-hayami` の例と同程度のキャラクターシートを作る。
...
1. `theo-hayami` のシートを基準にする。
2. 10人カードから採用できる方向性を拾う。
3. 各キャラのカード画像を reference image として渡し、同じ人物をシートへ展開する。
...
- 口調・能力名と矛盾していない。
- 元カードと別人になっていない。髪・目・表情・衣装色・持ち物の同一性を最優先する。
