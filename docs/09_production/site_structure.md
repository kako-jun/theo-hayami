...
| ルート | 内容 |
|---|---|
| `/`（`index.astro`） | 入口ヒーロー。集合キービジュアルを表紙として使い、第一導線を「本編」に置く。Reading Guide は `001 / READING GUIDE` の下に `002 / STORY`、`003 / FREE ACTION`、`004 / TEA TIME` を並べる。`じっくり読む時間がないときは` は `003 / FREE ACTION` の補足文として残し、`004 / TEA TIME` には「いまの話題が、住人たちのお茶の席に並ぶ」補足を添える |
...
- **作者導線**: `https://llll-ll.com` へのリンク＋ `© kako-jun`。GitHub Sponsors 等の支援導線は含めない（作者サイトのみに絞る・kako-jun 合意済み）。現実視点の案内として `/about` へのリンクも同じ行に置く。項目間の区切りは文字列の `──` ではなく CSS 疑似要素の細線で表現し、フォント字間に依存させない。
