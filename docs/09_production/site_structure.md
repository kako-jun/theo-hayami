...
| `/tea-time/community`（`tea-time/community.astro`） | **みんなの声**（Issue #131）。`004 / TEA TIME` の枝ページで独立folioは持たない。上部に好きな住人ランキング（Nostalgic Ranking、ID `theo-hayami-fc727d17`。read-onlyのembed componentに自前の投票UIを添える）、下部にお題投稿BBS（Nostalgic BBS、同ID。post機能込みのembed componentをそのまま埋め込む）を並べる。投票の判定・URL組み立ては `src/lib/rankingVote.ts` の純粋関数に切り出す。`/tea-time` の導入文直後に誘導カードを置く |
