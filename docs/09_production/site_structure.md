...
全ページ共通の下部情報は `src/components/SiteFooter.astro`（`LibraryLayout.astro` が全ページで使う）に集約する。既存のパンくず「叡智の星海 ── せおはやみ」の直下に、横線区切りなしで QR・作者導線・訪問カウンタを並べる（`.th-hairline` は初期実装で使ったが後の修正で撤去済み・書架の奥付のように控えめに、汎用UI chrome・ガラス板パネルは使わず生成絵に直接乗せて `.th-glow` の text-shadow だけで可読性を担保する）。
