...
| `/tea-time/community`（`tea-time/community.astro`） | **星海への接続端末**（Issue #131）。`004 / TEA TIME` の枝ページで独立folioは持たない。上部に推したい住人ランキング（Nostalgic Ranking API、ID `theo-hayami-fc727d17`）、下部にお題募集BBS（Nostalgic BBS、同ID。post機能込みのembed componentをそのまま埋め込む）を並べる。`<nostalgic-ranking>` のread-only widgetはname/score/rankのテキストのみで住人の顔を出せずピンとこないため使わず、ランキングは住人8人＋せお＋ヴィンチアの計10件固定（顔アップ・順位・名前・スコア・投票ボタン）で自前レンダリングする（`VOTE_TARGETS`＝`src/lib/rankingVote.ts` が正本。せお・ヴィンチアは専用bust.webpが無いため仮素材`normal.webp`＋基本形`.th-face-crop`を使用、kako-junが後日専用bustへ差し替え予定）。投票の判定・URL組み立て・行データ組み立ては `src/lib/rankingVote.ts` の純粋関数に切り出す。`/tea-time` の導入文直後に誘導カードを置く |
