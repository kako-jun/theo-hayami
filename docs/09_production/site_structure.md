...
| `/story`（`story.astro`） | 本編の幕別ボタン棚。冒頭の本編導入用集合絵には説明文を重ねず、下に `002 / Story` と幕カテゴリを置く。最初は01だけ押せ、読了すると次の本編ボタンが解放される。現段階では `main/ohako-*.md` 8本を、住人と初めて会う第一幕の本編ボタンとして扱う。ボタンごとの文量は揃えなくてよく、おはこ部分は短くてよい |
...
既読は「達成」なので**暗くしない**（トーン落とし＝消化済みのネガ印象は不採用・kako-jun 指示）。個別セルには温かい金の灯り（金 border＋金グロー）をともし、角に金地の「読了」蔵書印（`::after`ではなく`ReadMarks.astro`が挿す実DOM要素 `.th-read-stamp`・金地×濃墨＝押された勲章）を押す。面は沈めない。全読了カードは金の灯りを強める。集計カードの既読ゲージ+分数バッジ（`.th-read-meter`「N/M 読了」）は金で静かに。派手にしない（#20: 汎用 UI chrome 禁止）。
