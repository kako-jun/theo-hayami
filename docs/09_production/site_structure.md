...
| ルート | 内容 |
|---|---|
| `/`（`index.astro`） | 入口ヒーロー。集合キービジュアルを表紙として使い、2つの扉「住人を選ぶ」「物語を辿る」＋読み方説明 |
...
- **集計カード**に `data-read-group="slug,slug,..."`（束ねる読むページ群）と `[data-read-count]`（隠しバッジ）を持たせ、既読数を数えて「N/M 読了」を出す。全読了なら `.is-read-all`。対象は `anthology.astro`（住人カード＝その住人の全業）と `themes/index.astro`（業カード＝その業に答える全住人）。
...
- 正式なキャラ名タイポグラフィ画像はまだ未配置。仮画像を生成せず、当面は CSS の明朝・細罫・ノンブル風ラベルで誌面感を作る。
