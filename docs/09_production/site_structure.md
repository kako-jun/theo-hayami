...
| ルート | 内容 |
|---|---|
| `/`（`index.astro`） | 入口ヒーロー。集合キービジュアルを表紙として使い、第一導線を「本編」に置く。Reading Guide は `001 / Reading Guide` の下に `002 / Story`、`003 / Free Action`、`004 / Tea Time` を並べる。`じっくり読む時間がないときは` は `003 / Free Action` の補足文として残し、`004 / Tea Time` には「いまの話題を、住人たちがお茶の席で眺める」補足を添える |
...
| `/story`（`story.astro`） | 本編の幕別ボタン棚。冒頭の本編導入用集合絵には `002 / Story` と `物語を辿る` のタイトルだけを重ね、暫定説明文は重ねない。最初は01だけ押せ、読了すると次の本編ボタンが解放される。現段階では `main/ohako-*.md` 8本を、住人と初めて会う第一幕の本編ボタンとして扱う。ボタンごとの文量は揃えなくてよく、おはこ部分は短くてよい |
...
- **`public/manifest.webmanifest`**: 静的ファイルとして置く。`theme_color` / `background_color` は両方 `#0a0910`（`--color-th-ink-deep`。html/body の実背景と同じ）に揃える。別値にすると Android スプラッシュのステータスバーと本体の境目に薄い線が出る（hanoba #478 の教訓）。`LibraryLayout.astro` の `<meta name="theme-color">` も同じ値に揃えてある。icons は `favicon-192x192.png` / `favicon-512x512.png`（`purpose: "any"` のPNG）2つだけ。maskable・SVGエントリは入れない（hanoba #511/#513 の教訓）。
