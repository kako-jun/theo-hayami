...
mypace（`repos/2025/mypace/apps/web`）方式に揃える。hanoba は `generateSW` + `workbox` 設定 + `registerType: "autoUpdate"` だが、本サイトは `injectManifest` + `registerType: "prompt"` + 手書き SW + `virtual:pwa-register` の手動登録（更新 overlay ＋ 自動 reload）にする。

- **依存**: `@vite-pwa/astro`（Astro 用ラッパー）+ `workbox-precaching`（`src/sw.ts` が直接 import するため明示的な devDependency にした）。
- **`astro.config.mjs`**: `AstroPWA({ registerType: "prompt", strategies: "injectManifest", ... })`。
  - `injectRegister: false` を明示。デフォルトの `"auto"`（＝ソース中の `virtual:pwa-register` import を検出できれば自動登録スクリプトを止める）は、Astro のビルドがクライアントバンドル用とプリレンダー用で Vite の実行を分けているため伝播せず、`dist/registerSW.js`（素の register 版）が二重生成された（実機で確認済み）。
  - `injectManifest.globPatterns` は `**/*.{js,css,html,svg,woff2}`（アプリシェルのみ）。画像（webp/png）は含めない。サイト全体の背景画像だけで35MB超（296組の読むページがそれぞれ全画面背景を持つ）あり、SW install 時に全部precacheすると初回が重すぎる上、workbox の既定 `maximumFileSizeToCacheInBytes`（2MiB）を超える個別画像（例: `images/reality/city.png` 2.57MB）があると `injectManifest` がビルドエラーで落ちる（実機で確認済み）。本文の生成絵は通常のネットワークナビゲーション＋ブラウザHTTPキャッシュに任せる。
  - `manifest.theme_color` / `background_color` は両方 `#0a0910`（`--color-th-ink-deep`。html/body の実背景と同じ）に揃える。別値にすると Android スプラッシュのステータスバーと本体の境目に薄い線が出る（hanoba #478 の教訓）。`LibraryLayout.astro` の `<meta name="theme-color">` も同じ値に揃えてある。
  - `manifest.icons` は `favicon-192x192.png` / `favicon-512x512.png`（`purpose: "any"` のPNG）2つだけ。maskable・SVGエントリは入れない（Android起動スプラッシュでアイコン周囲に薄い枠が出る既知不具合の回避。hanoba #511/#513 で確定した原因）。
...
- **`LibraryLayout.astro`**: body 末尾の module `<script>`（既存のスクロール連動ズームと同じ作法）で `virtual:pwa-register` の `registerSW()` を呼ぶ。`onNeedRefresh` で cooldown 判定→ overlay 表示（`.th-pwa-overlay` / `.th-pwa-overlay__panel th-panel`、th-panel と同じガラス＋真鍮トーン）→ `controllerchange` リスナー設置 → `updateSW(true)`（skipWaiting）→ reload。`controllerchange` が来ない環境向けに2秒後の fallback reload も用意する。`<head>` には `@vite-pwa/astro` が注入しない `<link rel="manifest" href="/manifest.webmanifest" />` を明示で追加する。
