...
| `/residents/<slug>`（`residents/[character].astro`） | 住人大判キービジュアル＋プロフィール＋その住人が答える業一覧。業一覧は `/themes` と同じカテゴリにまとめる |
...
| `/story`（`story.astro`） | 本編（四幕）。**プレースホルダ**（Issue #20 層4・別PR）。準備中の間も自由行動へ戻れるよう、住人から選ぶ `/anthology` と業から選ぶ `/themes` の2導線を並べる。冒頭に本編導入用の集合絵を置き、先に読める8編はPCでも2列にして題名が中途半端に折り返さない幅を確保する |
...
- `/themes/<theme>` の住人選択ボタンは、`assets/images/theme-residents/<resident>.webp` の共通 960x540 顔画像を使う。横長ボタンは白地で統一し、画像を右側へ住人ごとに位置補正して置く。左端からなだらかな黒グラデーションを重ね、白文字の可読性を保ちながら顔と文字が重ならないようにする。
