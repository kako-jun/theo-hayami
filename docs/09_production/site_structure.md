...
- **QR コード**: `assets/images/ui/qr-code.webp`（`https://theo-hayami.llll-ll.com/` を指す、kako-jun が用意した意匠。中央に T ロゴを配置）を他の画像アセットと同じ `scripts/sync-assets.mjs` の仕組みで `/images/ui/qr-code.webp` として配信する。幅120px・`loading="lazy"`。画像自体は透過背景・白モジュールのみで、背景色は CSS 側（`.th-footer__qr { background: var(--color-th-ink); }`）が持つ。金枠は初期実装で使ったが後の修正で撤去済み。
