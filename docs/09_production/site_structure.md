...
| ルート | 内容 |
|---|---|
| `/`（`index.astro`） | 入口ヒーロー。集合キービジュアルを表紙として使い、第一導線を「本編」に置く。Reading Guide は `001 / Reading Guide` の下に `002 / Story`、`003 / Free Action`、`004 / Tea Time` を並べる。`じっくり読む時間がないときは` は `003 / Free Action` の補足文として残す |
