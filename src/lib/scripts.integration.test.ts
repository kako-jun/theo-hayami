...
describe("loadStoryButtons: 第一幕〜第四幕シーケンスの順序ゲート", () => {
  ...
  it("[act1-01, act1-02, おはこ×RESIDENTS順, act1-03, act1-04, act2-01..act4-08] の通し28件を並べる", () => {
    const expected = [
      "act1-01",
      "act1-02",
  ...
  it("幕内の表示順は幕ごとに1始まりへリセットする通し番号（第一幕1..12、第二/第三幕1..4、第四幕1..8）", () => {
    ...
    expect(buttons.slice(20, 28).map((entry) => entry.orderInAct)).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);
  });
...
describe("groupStoryButtonsByAct: /story の幕見出し分割", () => {
  it("28件フラット配列を第一幕12件・第二/第三幕4件・第四幕8件の4グループへ分ける（おはこは直前の act に属す）", () => {
    ...
    expect(sections[3]?.buttons.map((e) => e.slug)).toEqual([
      "act4-01",
      "act4-02",
      "act4-03",
      "act4-04",
