...
// Issue #43 / #46 / #50: セオ登場を「背景 → 表示完了待機 → 登場セオ → 表示完了待機 → 話者」の順に立てる間合いを構造として固定する安全網。
// content/scripts/**/*.md を実ファイルとして走査し、`[登場: セオ ...]` で始まる行について
//   - 直前の行が待機ディレクティブ `[待機: 表示完了]`（背景表示完了待ち・#50）
//   - 直後の行が待機ディレクティブ `[待機: 表示完了]`（セオ登場表示完了待ち・#50）
//   - 直前の直前（2行上）が `[背景:` で始まる（背景→待機→登場セオ の並び・#46）
// であることを検証する。
// 狙いは「背景（フェードイン）→ セオ（フェードイン）→ 最初の話者（ヴィンチア）」の間合いが、
// 将来スクリプトを再生成・編集した時に消える回帰を検出すること。
//
// なぜ直前にも待機が要るか（#46）:
// - name-name は非同期が既定＝連続ディレクティブは並行フェードで「同時」に見える。
//   逐次（背景を先に立ててからセオ）にするため `[待機: 表示完了]` で合流する。
...
// - `[登場: セオ` 行を持たないファイル（セオ開幕セル等）は自然に検査対象外になる＝意図どおり。
...
interface SeoEntrance {
  ...
  line: number; // 1-indexed の `[登場: セオ ...]` 行番号
...
// セオ登場行を全ファイルから収集する（1ファイルに複数あれば各々を独立して拾う）。
...
function waitViolation(
  ...
  if (target === undefined) {
    return `${entrance.file}:${entrance.line} 「セオ登場」の${where}に待機ディレクティブが無い（境界超え/末尾）`;
  }
...
describe("Issue #43/#46/#50: セオ登場の前後の表示完了待機ディレクティブ（背景→待機→登場→待機の間合い）", () => {
  it("走査対象の *.md が存在する（glob/パス破損で空集合になる偽陰性のガード）", () => {
    expect(MD_FILES.length).toBeGreaterThan(0);
  });

  it("セオ登場行が1件以上見つかる（検査対象が空で全部pass する偽陰性のガード）", () => {
    // ここが 0 だと以降の不変条件テストは vacuously true になり回帰を見逃す。
    expect(SEO_ENTRANCES.length).toBeGreaterThan(0);
  });

  it("`[登場: セオ` で始まる各行の直後が待機ディレクティブ `[待機: 表示完了]` である（#50 セオ登場表示完了待ち）", () => {
    // 1ファイルに複数のセオ登場があっても、収集時に各出現を独立して拾っているため
  ...
  it("`[登場: セオ` で始まる各行の直前も待機ディレクティブ `[待機: 表示完了]` である（#50 背景表示完了待ち）", () => {
    const violations = SEO_ENTRANCES.flatMap((e) => {
  ...
  it("`[登場: セオ` の2行上が `[背景:` で始まる（背景 → 待機 → 登場セオ の並び・#46）", () => {
    const violations = SEO_ENTRANCES.flatMap((e) => {
      if (e.prev2 === undefined) {
        return [`${e.file}:${e.line} 「セオ登場」の2行上が無い（境界超え）`];
      }
